...
// generateSalt
std::string bcrypt::generateSalt (unsigned int rounds) 
{
	std::string salt(32, '\0');
	char salt_arr[_SALT_LEN];
	unsigned char seed[17]{};
	arc4random_init();
	
  arc4random_buf(seed, 16);

  bcrypt_gensalt('b', rounds, seed, salt_arr);
	salt = salt_arr;

	return salt;
}

// generateHash from plain password & salt
std::string bcrypt::generateHash (const std::string &plain_passwd, const std::string& salt ) 
{
    std::string hash(61, '\0');				
    node_bcrypt(plain_passwd.c_str(), plain_passwd.size(), salt.c_str(), &hash[0]);
    hash.resize(60);
    return hash;
}

// validatePassword by comparing chk_hash to original hash
// - more secure since the plain password is not recv'd for validation
// - chk_hash must be salted with the same orig salt
bool bcrypt::validatePassword(const std::string &chk_hash, const std::string &orig_hash) 
{
    return chk_hash == orig_hash;
}

// validatePlainPassword using a plain chk passwd 
bool bcrypt::validatePlainPassword ( const std::string &plain_passwd, const std::string &orig_hash) 
{
		std::string chk_hash(61, '\0');
		std::string salt(32, '\0');
		salt = orig_hash;
    node_bcrypt ( plain_passwd.c_str(),plain_passwd.size(), salt.c_str(), &chk_hash[0]);
    chk_hash.resize(60);
    return chk_hash == orig_hash;
}
